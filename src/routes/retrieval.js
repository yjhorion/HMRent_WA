    ...
    try{
        ...
        const sendingdata = JSON.stringify({
            ...
            "data" : {
                ...
                "QRDATBEG" : QRDATBEG, // 조회일자시작일 - YYYYMMDD 포멧으로 프론트에서 값을 받아서 전달. reqbody에 해당  값이 빈 string 혹은 null 일경우, 현재날자 값
        ...
        res.send({
            data : JSON.parse(router.get('/retrieval', async (req, res, next) => {
    try{
        const { year, month, day, hour, minute, second } = getCurrentDateTime();
        // const userAgent = req.headers['user-agent']; 
        // const deviceType = getDeviceType(userAgent);
