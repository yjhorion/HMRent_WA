 # 1. (해결). 이미지 다중 업로드시 데이터 유실 발생
## => message queue를 이용해서 순차적으로 전송하는 방식을 시도중 => 이미지파일을 배열에 담은 이후, sqs로한번에 보내는 방식으로 다시 시도중
## (해결) => MQ를 사용해봤으나 해결되지 않음(카카오톡 이미지 전송처럼 FIFO형태로 하나씩 압축하고 전송하고 완료되면 새롭게 전송하는 형태를 시도했으나, MQ를 적용해도 여전히 데이터 유실이 발생하였고, 애시당초 multer.array() 자체가 데이터를 배열에 담아 전송하는 형태로 작동함. 결과적으로는, 파일명이 겹치는경우의 수가 원인이었다고 판단되어, uuidv4를 적용하였고, directory가 없는 bucket의 특성상, 파일명에 shortform uuiv + 디렉토리명으로 파일을 분류. 유실없이 전달되고있음. uuiv가 shortform이기에, 일일업로드 데이터가 많아지면 중복가능성 상승. 추후 개선이 필요할 수 있음.

# 1-1. (해결) 이미지 파일의 파일명이 깨져서 나오는 현상 => 해결하는김에 원하는 디렉토리 형식으로 파일명을 저장시도중. 
## ('차량번호'_'YYYY/MM/DD'_'uuid') 형식으로 저장하여, 차량번호 + 날자 단위로 조회하거나 sort 할 수 있도록. 이미지 파일명이 한글일 경우, 깨져나오는 현상. html 의 lang option이 en으로 설정되어있던것이 이유로 판단. html의 lang = kr/en 으로 설정하고, 저장되는 파일명을 originalfilename을 사용하지 않는 임의로 저장값을 지정하여 저장. 해결.
## +@ 업로드된 이미지 파일의 미리보기값 저장. (bucket에서 파일 다운로드 형식이 아닌 미리보기를 위해 contentType: multerS3.AUTO_CONTENT_TYPE 으로 지정해 프론트에서 전달된 json에서 link를 불러와 미리보기 구현.
